...
const linkBooks = async (prevBookTitle: string, bookTitle: string) => {
  let additionalCommands: string[] = []

  additionalCommands.push(`MATCH (prevBook:Book {title: "${prevBookTitle}"})`)
  ...
  try {
    await session.executeWrite(async (tx) => {
      ...
      await fs.appendFile(logFile, `***** LINK BOOK QUERY *****\n${result.summary.query.text}\n\n`)
    })
  } catch (error: any) {
    await fs.appendFile(
      errorFile,
...
const run = async () => {
  ...
  for (const [volumeTitle, mappedBook] of Object.entries(mappedLdsVerses)) {
    ...
    for (const [bookTitle, verses] of Object.entries(mappedBook)) {
      ...
      for (const verse of verses) {
        ...
        if (!isSameChapter && lastChapterNumber !== 0) {
          try {
            await writeChapter({ verseCommands, parentCommands }, params, prevVerseTitle)
        ...
        const { chapter_number, scripture_text, verse_number, verse_short_title, verse_title } =
          verse
        params[paramName] = {
          chapter_number,
          scripture_text,
      ...
      if (prevBookTitle) {
        linkBooks(prevBookTitle, bookTitle)
      }
    ...
    await fs.appendFile(logFile, `***** VOLUME VERSE COUNT: ${volumeVerseCount} *****\n\n`)
  }
